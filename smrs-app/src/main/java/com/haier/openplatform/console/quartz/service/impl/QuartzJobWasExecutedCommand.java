 ...
 *
 */
public class QuartzJobWasExecutedCommand extends QuartzCommand {
	private static final Logger LOGGER = LoggerFactory.getLogger(QuartzJobWasExecutedCommand.class);
	...
	public void execute(Map<String, String> dataMap) {
		...
		if (dbJobPlan == null) {
			LOGGER.error("Job plan is null,schedulerName:{},jobGroup:{},jobName:{},cronExpression{}", schedulerName,
					jobGroup, jobName, cronExpression);
			dbJobPlan = new JobPlan();
			dbJobPlan.setId(-1L);
		} 
		JobTrace jobTrace = JobMapUtils.createJobTrace(dataMap);
		jobTrace.setState("jobWasExecuted");
