...
function searchForm(){
	document.getElementById('updateForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/relationship/searchCustomer.action";
}
</script>
...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="updateCustomer" namespace="/relationship" method="post" id="updateForm">
				<s:hidden name="customer.id"/>
				<s:hidden name="customer.creator"/>
				<s:hidden name="customer.status"/>
				<s:hidden name="actionCommand" value="update"/>
				<%@include file="customerInc.jsp"%>					
	
				<div class="row">
					<div class="tableLeft">
						<label>创建者</label>
						<label><s:property value="customer.creator" /></label>
					</div>
					<div class="tableRight">
						<label>创建时间</label>
						<label><s:date name="customer.creationDate" format="yyyy-MM-dd HH:mm:ss" /></label>
					</div>
				</div>
