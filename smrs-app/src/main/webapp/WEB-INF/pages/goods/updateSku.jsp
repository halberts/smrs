...
function searchForm(){
	document.getElementById('updateForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/searchSku.action";
}
</script>
</head>
<body>
...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="updateSku" namespace="/goods" method="post" id="updateForm">
				<s:hidden name="goodsSku.id"/>
				<s:hidden name="goodsSku.creator"/>
				<s:hidden name="actionCommand" value="update"/>
				<%@include file="skuInc.jsp"%>					
	
				<div class="row">
					<div class="tableLeft">
						<label>创建者</label>
						<label><s:property value="goodsSku.creator" /></label>
					</div>
					<div class="tableRight">
						<label>创建时间</label>
						<label><s:date name="goodsSku.creationDate" format="yyyy-MM-dd HH:mm:ss" /></label>
					</div>
				</div>
