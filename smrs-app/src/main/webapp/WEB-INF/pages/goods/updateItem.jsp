...
function searchForm(){
	document.getElementById('updateForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/searchItem.action";
}
</script>
</head>
<body>
...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="updateItem" namespace="/goods" method="post" id="updateForm">
				<s:hidden name="goodsItem.id"/>
				<s:hidden name="goodsItem.creator"/>
				<s:hidden name="actionCommand" value="update"/>
				<%@include file="itemInc.jsp"%>					
	
				<div class="row">
					<div class="tableLeft">
						<label>创建者</label>
						<label><s:property value="goodsItem.creator" /></label>
					</div>
					<div class="tableRight">
						<label>创建时间</label>
						<label><s:date name="goodsItem.creationDate" format="yyyy-MM-dd HH:mm:ss" /></label>
					</div>
				</div>
