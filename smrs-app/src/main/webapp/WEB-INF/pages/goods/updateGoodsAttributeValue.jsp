...
function searchForm(){
	document.getElementById('updateForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/searchGoodsAttributeValue.action";
}
</script>
</head>
<body>
...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="updateGoodsAttributeValue" namespace="/goods" method="post" id="updateForm">
				<s:hidden name="goodsAttributeValue.id"/>
				<s:hidden name="goodsAttributeValue.creator"/>
				<s:hidden name="actionCommand" value="update"/>
				<%@include file="goodsAttributeValueInc.jsp"%>					
	
				<div class="row">
					<div class="tableLeft">
						<label>创建者</label>
						<label><s:property value="goodsAttributeValue.creator" /></label>
					</div>
					<div class="tableRight">
						<label>创建时间</label>
						<label><s:date name="goodsAttributeValue.creationDate" format="yyyy-MM-dd HH:mm:ss" /></label>
					</div>
				</div>
