...
function searchForm(){
	document.getElementById('updateForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/searchSupplier.action";
}
</script>
...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="updateSupplier" namespace="/goods" method="post" id="updateForm">
				<s:hidden name="supplier.id"/>
				<s:hidden name="supplier.creator"/>
				
				<s:hidden name="actionCommand" value="update"/>
				<%@include file="supplierInc.jsp"%>					
	
				<div class="row">
					<div class="tableLeft">
						<label>创建者</label>
						<label><s:property value="supplier.creator" /></label>
					</div>
					<div class="tableRight">
						<label>创建时间</label>
						<label><s:date name="supplier.creationDate" format="yyyy-MM-dd HH:mm:ss" /></label>
					</div>
				</div>
