...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="searchGoodsCategory" namespace="/goods" method="get" id="searchForm">
				<table>
					<tr>
						<th>名称：</th>
						<th><s:textfield name="goodsCategory.name" /></th>
						<th>
							<button type="submit" class="orange" onclick="submitForm('searchGoodsCategory');"><span>查询</span></button>
					...
					<s:iterator value="pager.records" var="goodsAttribute" status="status">
						<tr>
							...
							<td>
								...
								<img title="添加子类" border="0" src="${staticURL}/images/add.jpg" 	onclick="addForm(<s:property value="id"/>)">							
							</td>
...
function delForm(id){
	...
	window.location.href = "${dynamicURL}/goods/deleteGoodsCategory.action?goodsCategory.id="+id;
}

function addForm(id){
	window.location.href = "${dynamicURL}/goods/addGoodsCategory.action?goodsCategory.parent.id="+id;
}

//删除
function updateForm(id){
	window.location.href = "${dynamicURL}/goods/updateGoodsCategory.action?goodsCategory.id="+id;
}

function createForm(){
	document.getElementById('searchForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/addGoodsCategory.action";
}
