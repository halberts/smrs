...
<div class="section">
	...
	<div class="box">
		...
		<div class="content">
			<s:form action="searchSupplier" namespace="/goods" method="get" id="searchForm">
				<table>
					<tr>
						<th>名称：</th>
						<th><s:textfield name="supplier.name" /></th>
						<th>
							<button type="submit" class="orange" onclick="submitForm('searchSupplier');"><span>查询</span></button>
	...
	<div class="box">
		...
		<div class="content">
			...
			<table class="sorting"> 
				...
				<tbody>
					<!-- 数据行 -->
					<s:iterator value="pager.records" var="relationshipSupplier" status="status">
						<tr>
							<td ><a	href='#' onclick="updateForm(<s:property value="id"/>)"><s:property value="name" /></a></td>
							<td><s:property value="code" /></td>							
							<td><s:property value="@com.smrs.goods.enums.SupplierTypeEnum@toEnum(type).name" /></td>
...
function delForm(id){
	...
	window.location.href = "${dynamicURL}/goods/deleteSupplier.action?supplier.id="+id;
}

//删除
function updateForm(id){
	window.location.href = "${dynamicURL}/goods/updateSupplier.action?supplier.id="+id;
}

function createForm(){
	document.getElementById('searchForm').onsubmit=function(){ return false; };
	window.location.href = "${dynamicURL}/goods/addSupplier.action";
}
